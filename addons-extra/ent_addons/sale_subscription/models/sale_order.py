...
class SaleOrder(models.Model):
    ...
    def _handle_subscription_payment_failure(self, invoice, transaction, email_context):
        ...
        invoice.unlink()
        auto_close_days = self.sale_order_template_id.auto_close_limit or 3650
    ...
    def _get_expired_subscriptions(self):
        ...
        self.env.cr.execute(
            """
                SELECT (so.next_invoice_date + INTERVAL '1 day' * COALESCE(sot.auto_close_limit,3650)) AS "payment_limit",
             ...
             LEFT JOIN sale_order_template sot ON sot.id=so.sale_order_template_id
                ...
                AND (so.next_invoice_date + INTERVAL '1 day' * COALESCE(sot.auto_close_limit,3650))< %s
        ...
        self.env.cr.execute(
            """
                WITH payment_limit_query AS (
                      SELECT (aml2.dm + INTERVAL '1 day' * COALESCE(sot.auto_close_limit,3650) ) AS "payment_limit",
