...
class OAuthController(http.Controller):
    ...
    @http.route('/crm_meta_leads/auth', type='http', auth='user', website='False')
    @fragment_to_query_string
    def add_access_token(self, **kw):
        if kw.get('access_token'):
            params = {
                'client_id': request.env['ir.config_parameter'].sudo().get_param('crm_meta_leads.crm_fb_app_id'),
                'client_secret': request.env['ir.config_parameter'].sudo().get_param(
                    'crm_meta_leads.crm_fb_app_secret'),
                'fb_exchange_token': kw.get('access_token')
            }
            r = requests.get("https://graph.facebook.com/v15.0/oauth/access_token?grant_type=fb_exchange_token",
                             params=params).json()
            if r.get('error'):
                _logger.log(r.get('error').get('message'))
            request.env['ir.config_parameter'].set_param("crm_meta_leads.crm_fb_access_token",
                                                         r.get('access_token', ''))
