...
class ResConfigSettings(models.TransientModel):
    ...
    crm_fb_app_id = fields.Char('App ID', config_parameter='crm_meta_leads.crm_fb_app_id')
    crm_fb_app_secret = fields.Char('App Secret', config_parameter='crm_meta_leads.crm_fb_app_secret')
    crm_fb_access_token = fields.Char('Access Token', config_parameter='crm_meta_leads.crm_fb_access_token')
    ...
    def action_get_access_token(self):
        redirect_url = "%s/crm_meta_leads/auth" % (self.env['ir.config_parameter'].get_param('web.base.url'))
    ...
    def action_get_facebook_pages(self):
        ...
        action = self.env.ref('crm_meta_leads.action_crm_facebook_page').read()[0]
        return action
    ...
    def _get_access_token_state(self):
        ...
        if not (self.crm_fb_app_id and self.crm_fb_app_secret):
            ...
            return
        r = requests.get("https://graph.facebook.com/v15.0/debug_token", params={'input_token': self.crm_fb_access_token,
                                                                                'access_token': '|'.join(
                                                                                    [self.crm_fb_app_id,
