...
class AppointmentAccountPayment(AppointmentController):
    ...
    def appointment_form_submit(
        ...
        """
        Sobrescritura: cuando es necesario un paso de pago, se crea el modelo de reserva
        de cita para almacenar toda la información relevante en lugar de crear un
        `calendar.event`. Este modelo será transformado en un `calendar.event` durante
        el pago (o la confirmación). Ver `make_event` en `appointment.booking`.
        Redirige al pago si es necesario. Ver `redirect_to_payment`.
        """
        appointment = request.env['appointment.type'].sudo().browse(appointment_type_id)
        partner = request.env['res.partner'].search([('email', '=like', email)], limit=1)
        if not partner:
        ...
        if appointment.has_payment_step and appointment.product_id.lst_price:
            ...
            return request.render(
                "isep_appointment_payment.custom_appointment_payment", values)
    ...
    def appointment_payment(self, appointment_id, **kwargs):
        """
        Se crea un evento después del paso de pago y se redirige a la página de
        confirmación/resumen de la reserva.
        """
        appointment = request.env['appointment.type'].browse(appointment_id)
        ...
        description_bits = []
        description = ''
        description_bits.append(_('Móvil: %s', kwargs.get('phone')))
        description_bits.append(_('Correo electrónico: %s', kwargs.get('mail')))
        description = '<ul>' + ''.join(
...
class AppointmentCancel(AppointmentCalendarController):
    ...
    def appointment_cancel(self, access_token, partner_id, **kwargs):
        """
        Si la cita es cancelada, se crea una nota de crédito.
        """
