...
class TeamsCalendarEvent(models.Model):
    ...
    teams_meeting_url = fields.Char(string="Teams Meeting URL")
    ...
    def _check_teams_bool(self):
        ...
        """
        if self.create_uid and self.teams_meeting_url:
    ...
    def create(self, values):
        ...
        for vals in values:
            if all([vals.get('teams_link_check'), vals.get('recurrency')]):
                ...
                vals['stop'] = end_time
                vals['teams_meeting_url'] = meeting_link.get('meeting_url')
    ...
    def write(self, values):
        ...
        for record in self:
            ...
            if all([is_meeting, event_recurrency]):
            ...
            elif values.get('teams_link_check') and not record.teams_meeting_url:
                if not active_user.is_authenticated:
                    raise ValidationError("Generate an access token to create a Microsoft Teams meeting.")
                ...
                for vals in record.read([]):
                    ...
                    values['teams_meeting_url'] = meeting_link.get('meeting_url')
                ...
                if record.write_uid == active_user:
                    ...
                    values['description'] = ''
                    record.teams_meeting_url = False
                else:
                    raise UserError(
                        "This Event can only Delete by Meeting Organizer")

            elif record.teams_link_check and record.teams_meeting_url:
                if not active_user.is_authenticated:
    ...
    def generate_meeting(self, values, attendees, user):
        ...
        payload = {
            ...
            "body": {
                "contentType": "HTML",
                "content": "" if not values.get(
                    'description') else values.get('description')
                },
            "start": {
