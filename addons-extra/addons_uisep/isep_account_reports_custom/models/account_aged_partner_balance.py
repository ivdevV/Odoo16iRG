    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                query_res = query_res_lines[0]  # Solo hay un resultado al agrupar por ID

                # Fix: asegurar que currency_id sea int y no lista
                currency_id = (
                    query_res['currency_id'][0]
                    if isinstance(query_res.get('currency_id'), list) and len(query_res['currency_id']) == 1
                    else None
                )
                ...
                rslt.update({
                    ...
                    # Fix: dejar amount_currency como número (float), no string formateado
                    'amount_currency': query_res['amount_currency'],
                    # Opcional: puedes usar este campo para mostrar en la vista
                    'amount_currency_formatted': report.format_value(query_res['amount_currency'], currency=currency),
                    ...
                    # Necesario para el unfold_all
                    'partner_id': query_res['partner_id'][0] if query_res['partner_id'] else None,

                    # Campo personalizado tuyo
                    'token': 'Sí' if any(t for t in query_res.get('token', []) if t) else None,
                })
            else:
                rslt.update({
                    'due_date': None,
                    'amount_currency': None,
                    'amount_currency_formatted': None,
                    'currency': None,
                    'currency_id': None,
                    'account_name': None,
                    'expected_date': None,
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT order_sale.payment_token_id) AS token,
            ...
            JOIN account_move move_account ON move_account.id = account_move_line.move_id
            LEFT JOIN sale_order order_sale
                ON order_sale.id = move_account.order_subscription_id
