...
(function(){
  ...
  // reenvíos desde el padre (opción B)
...
<body>${slide.htmlContent}</body>
        ...
        this._onEmbedMessage = (event) => {
            if (event.source !== this._fsIframe?.contentWindow) return;
        ...
        const onParentResize = () => { this._forwardResize(); this._requestChildMeasure(); };
    ...
    _requestChildMeasure() {
        try { this._fsIframe?.contentWindow?.postMessage({ __oembed_getHeight: true }, '*'); } catch(_) {}
    },
    _forwardScroll() {
        try { this._fsIframe?.contentWindow?.postMessage({ __oembed_forwardScroll: true }, '*'); } catch(_) {}
    },
    _forwardResize() {
        try { this._fsIframe?.contentWindow?.postMessage({ __oembed_forwardResize: true }, '*'); } catch(_) {}
    },

    _teardownEmbedHandlers() {
        if (this._onEmbedMessage) { window.removeEventListener('message', this._onEmbedMessage); this._onEmbedMessage = null; }
        if (this._scrollTargets?.length) {
            for (const [t, evt, h] of this._scrollTargets) { try { t.removeEventListener(evt, h); } catch(_) {} }
        }
        this._scrollTargets = [];
        this._fsIframe = null;
    },
