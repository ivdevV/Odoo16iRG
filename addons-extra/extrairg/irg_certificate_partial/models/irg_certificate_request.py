...
class IrgCertificateRequest(models.Model):
    ...
    @staticmethod
    def _ensure_bottom_right_arcs(tpl_path, docx_path):
        """Place the blue arc decoration in the lower-right page corner."""
        rel_ns = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships'
        rel_embed = '{%s}embed' % rel_ns
        rels_ns = 'http://schemas.openxmlformats.org/package/2006/relationships'

        with ZipFile(tpl_path) as template_zip:
            media_name = 'word/media/image1.png'
            if media_name not in template_zip.namelist():
                return
            media_data = template_zip.read(media_name)
        ...
        if output_xml.xpath('.//*[local-name()="docPr" and @name="Corner Arcs"]'):
            return

        rel_target = 'media/image1.png'
        output_rel = next(
            (rel for rel in output_rels if rel.get('Target') == rel_target),
            None,
        )
        if output_rel is None:
            rel_id = IrgCertificateRequest._next_relationship_id(output_rels)
            output_rel = etree.Element('{%s}Relationship' % rels_ns)
        ...
        docpr_id = IrgCertificateRequest._next_docpr_id(output_xml)
        drawing_xml = '''
<w:r xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
  ...
  <w:drawing>
    <wp:anchor distT="0" distB="0" distL="0" distR="0" simplePos="0"
      ...
      <wp:extent cx="3200000" cy="1545600"/>
      <wp:effectExtent l="0" t="0" r="0" b="0"/>
      <wp:wrapNone/>
      <wp:docPr id="%s" name="Corner Arcs"/>
      <wp:cNvGraphicFramePr/>
      <a:graphic>
        <a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture">
          <pic:pic>
            <pic:nvPicPr>
              <pic:cNvPr id="%s" name="Corner Arcs"/>
              <pic:cNvPicPr/>
              <pic:nvPr/>
            </pic:nvPicPr>
            ...
            <pic:spPr bwMode="auto">
              <a:xfrm><a:off x="0" y="0"/><a:ext cx="3200000" cy="1545600"/></a:xfrm>
...
''' % (docpr_id, docpr_id, rel_id)
        drawing_run = etree.fromstring(drawing_xml.encode('utf-8'))
        ...
        tmp_zip = tempfile.NamedTemporaryFile(
            suffix='.docx', delete=False, prefix='cert_partial_arcs_'
        )
        tmp_zip.close()
        try:
            with ZipFile(docx_path) as source_zip, ZipFile(
                ...
                if media_name not in existing_names:
                    target_zip.writestr(media_name, media_data)
            os.replace(tmp_zip.name, docx_path)
        finally:
            if os.path.exists(tmp_zip.name):
                os.unlink(tmp_zip.name)
    ...
    def _fill_template(self):
        ...
        self._ensure_bottom_right_arcs(tpl_path, tmp_docx.name)
