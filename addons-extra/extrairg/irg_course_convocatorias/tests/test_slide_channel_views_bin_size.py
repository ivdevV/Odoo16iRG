...
class TestSlideChannelViewsBinSize(unittest.TestCase):
    """Regresión estática para evitar cargar binarios reales en onchanges x2many."""

    @classmethod
    def setUpClass(cls):
        cls.view_path = Path(__file__).resolve().parents[1] / 'views' / 'slide_channel_views.xml'
    ...
    def _context_attribute_for_xpath(self, field_name):
        ...
        for xpath_node in self.root.findall('.//xpath'):
            if needle not in xpath_node.attrib.get('expr', ''):
                continue
            for attr_node in xpath_node.findall("field[@name='context']"):
    ...
    def test_homeclass_slide_ids_context_uses_bin_size_and_keeps_defaults(self):
        ...
        self.assertIn("'default_irg_content_modality': 'homeclass'", context)

    def test_homeclass_native_sections_context_uses_bin_size_and_keeps_defaults(self):
        context = self._context_attribute_for_xpath('irg_native_section_ids')
        self._assert_bin_size_true(context)
        self.assertIn("'default_channel_id': active_id", context)
        self.assertIn("'default_irg_content_modality': 'homeclass'", context)
    ...
    def test_online_slide_ids_context_uses_bin_size_and_keeps_defaults(self):
        context = self._field_context('irg_online_slide_ids')
        self._assert_bin_size_true(context)
        self.assertIn("'default_channel_id': parent.irg_online_channel_id", context)
        self.assertIn("'default_irg_content_modality': 'online'", context)

    def test_online_section_ids_context_uses_bin_size(self):
        context = self._field_context('irg_online_section_ids')
        self._assert_bin_size_true(context)
