...
Anade el campo `Fecha de inicio de clases` al formulario de admisiones de OpenEduCat (`op.admission`). El campo permite registrar una fecha propia de inicio de clases por admision, separada de la fecha de admision, la fecha de solicitud, la fecha de vencimiento y la fecha de inicio de cuotas.
...
- Campo de solo lectura cuando la admision esta en estado `done`, siguiendo el comportamiento de `admission_date`.
...
5. Revisar una admision en estado `done` y confirmar que el campo no es editable.
