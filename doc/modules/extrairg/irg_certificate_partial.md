...
- **Ajustes de Diseño en Plantilla Word:**
  ...
  - **Decoración inferior derecha:** Se inserta la imagen de arcos azules (`image1.png`) como anclaje de página en la esquina inferior derecha (`page/right` y `page/bottom`), por detrás del texto, para que no desplace el cierre ni la firma.
...
* **Reemplazo Dinámico (`_fill_template`):**
  ...
  - Inserta la decoración de esquina inferior derecha con `_ensure_bottom_right_arcs()`. El helper usa `image1.png` de la plantilla, crea una relación a `media/image1.png` cuando hace falta y añade un anclaje de página con `behindDoc="1"`, alineado a derecha e inferior de la hoja para no modificar la distribución del contenido.
...
- `test_02_partial_gradebook_first_sentence_has_bold_student_and_course`: Genera el certificado parcial y abre el `.docx` resultante para validar que la primera frase contiene el alumno y el curso esperados, y que ambos aparecen en runs independientes con `bold == True`. También comprueba en el XML final del `.docx` que el texto legal vertical conserva los marcadores `B56488687` y `B-603323`, que el textbox usa tamaño compacto `w:val="10"` y que la decoración inferior derecha se ancla a `page/right` y `page/bottom` con `image1.png`.
...
docker compose -f docker-compose.local.yml exec -T odoo_local odoo -c /etc/odoo/odoo.conf -d test_irg_certificate_partial_bottom_right_arcs_20260605 --test-enable --stop-after-init -i irg_certificate_partial --test-tags /irg_certificate_partial --http-port=8099 --log-level=test
```

Los comandos finalizaron correctamente. La validación Odoo reportó:

```text
odoo.tests.result: 0 failed, 0 error(s) of 5 tests when loading database 'test_irg_certificate_partial_bottom_right_arcs_20260605'
```
...
- **Decoración inferior derecha:** El gráfico de arcos azules se inserta como imagen anclada a la página en la esquina inferior derecha, usando `image1.png` y sin afectar al flujo del texto ni al bloque de firma.
