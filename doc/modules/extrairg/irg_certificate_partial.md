...
- **Personalización de la Carga Lectiva para el Máster ID 4:** Cuando el alumno cursa el Máster con ID 4 (*Neuropsicología Clínica basada en la Evidencia*), se adaptan dinámicamente los placeholders en el certificado parcial para reflejar una carga lectiva de 120 ECTS (equivalente a 3000 horas de estudio) y un período de duración de 2 años en el texto explicativo.
- **Ajustes de Diseño en Plantilla Word:**
  ...
  - **Normalización de alturas de fila en la tabla de notas:** Al igual que en el certificado final, las filas de datos de la tabla de notas del certificado parcial se normalizan dinámicamente a una altura de `315 dxa` (aprox. 15.75 pt) con la regla de altura `atLeast`, previniendo desajustes visuales y asegurando que las celdas de notas mantengan una proporción mínima uniforme pero adaptable.
  - **Firma digitalizada de Raimon Gaja:** En certificados firmados por Raimon Gaja (`signer == 'raimon'`), se invoca al helper heredado `_ensure_signature_logo()` para insertar de forma segura y sin duplicaciones la firma digitalizada `logodesgastado.png` al lado derecho de su firma manuscrita (`media/image2.*`) en el mismo párrafo, con un tamaño de 100 pt.
...
* **Reemplazo Dinámico (`_fill_template`):**
  ...
  - Si el firmante es Raimon Gaja (`signer == 'raimon'`), se invoca al helper `_ensure_signature_logo()` para añadir el logo digitalizado de su firma `logodesgastado.png` al lado derecho de su firma manuscrita (`media/image2.*`) en el mismo párrafo.
...
- **Carga lectiva del Máster ID 4:** Personalizada la carga lectiva (120 ECTS / 3000 horas de estudio) y período de duración (2 años) para el Máster con ID 4 (*Neuropsicología Clínica basada en la Evidencia*) en certificados parciales de notas.
- **Pruebas de regresión:** Añadido test automatizado de integración para verificar el formateo de 120 ECTS y 3000 horas de estudio en el Máster ID 4.
...
- **Inserción de firma digitalizada de Raimon Gaja:** Añadida la invocación a `_ensure_signature_logo()` para inyectar la firma desgastada digitalizada de Raimon Gaja (`logodesgastado.png`) al lado derecho de su firma manuscrita (`media/image2.*`) en el mismo párrafo, previniendo duplicidades y a un tamaño de 100 pt.
- **Normalización de alturas de fila:** Forzada la altura de las filas de datos de la tabla de notas parciales a `315 dxa` con la regla de altura `atLeast` para garantizar consistencia visual y corregir desalineaciones.
