...
- **2026-05-27 (Revisión V1.2)**: Corregido error `'Received unknown parameter: subscription_data[cancel_at]'` eliminando el parámetro en la creación del Enlace de Pago (no admitido por Stripe). El vencimiento automático (`cancel_at`) se asocia ahora actualizando la suscripción en Stripe vía API de forma asíncrona dentro del procesamiento del webhook `checkout.session.completed` cuando se completa la sesión de pago.
