...
## Module Commit Workflow

**Whenever creating or updating any `irg_*` module**, follow this exact sequence before committing and pushing:

### Step 1 — Invoke the documenter subagent
After finishing code changes in a module under `addons-extra/extrairg/`, invoke the
`Odoo 16 Module Documenter` subagent to generate or update its documentation:

> "Document the module `irg_<module_name>`"

The subagent will write/update `doc/modules/extrairg/irg_<module_name>.md` using the
format established in [doc/modules/extrairg/](doc/modules/extrairg/).

### Step 2 — Stage everything together
```bash
git add addons-extra/extrairg/irg_<module_name>/
git add doc/modules/extrairg/irg_<module_name>.md
```

### Step 3 — Single commit with code + docs
```bash
git commit -m "feat(irg_<module_name>): <description>

docs: auto-documented by Odoo 16 Module Documenter"
```
...
> **Rule:** Never push module code without its updated `doc/modules/extrairg/irg_<module_name>.md`.
> Code and documentation travel in the same commit.
